...
public class StackLogic extends PApplet {
...
class Mino
{
	...
	Mino( String type, int r )
	{
		...
		switch( type )
		{
			...
			case "Puttyo":
				block[0] = new Block( 1, 1, 0xffFFFFFF );
				block[1] = new Block( 2, 2, 0xffFFFFFF );
				block[2] = new Block( 3, 1, 0xffFFFFFF );
				block[3] = new Block( 4, 2, 0xffFFFFFF );
				block[4] = new Block( 1, 1, 0xffFFFFFF );
				block[5] = new Block( 2, 2, 0xffFFFFFF );
				block[6] = new Block( 3, 1, 0xffFFFFFF );
				block[7] = new Block( 4, 2, 0xffFFFFFF );
			break;
		}
	}
...
public void drawField()
{
	...
	if( stackMino != null ){
		...
		if( stackMino.getWidth()  % 2 == 0 ) 	translate( ( blockSize * 0.8f / 2 ), 0 );
		if( stackMino.getHeight() % 2 == 0 ) 	translate( 0, ( blockSize * 0.8f / 2 ) );
		if( stackMino.getWidth() == 4 ) 		translate( 0, ( blockSize * 0.8f ) );
		for( int i = 0; i < 4; i++ ){
			fill( stackMino.getColor( i ) );
			rect( blockSize * 0.8f * ( stackMino.getX( i ) - 5 ) + 2, blockSize * 0.8f * ( stackMino.getY( i ) - 1 ) - 1, blockSize * 0.8f, blockSize * 0.8f );
	...
	for( int i = 1; i < 5; i++ ){
		...
		if( nextMino[i].getWidth()  % 2 == 0 ) 	translate( ( blockSize * 0.8f / 2 ), 0 );
		if( nextMino[i].getHeight() % 2 == 0 ) 	translate( 0, ( blockSize * 0.8f / 2 ) );
		if( nextMino[i].getWidth() == 4 ) 		translate( 0, ( blockSize * 0.8f ) );
		for( int j = 0; j < 4; j++ ){
			fill( nextMino[i].getColor( j ) );
			rect( blockSize * 0.8f * ( nextMino[i].getX( j ) - 5 ) + 2, blockSize * 0.8f * ( nextMino[i].getY( j ) - 1 ) - 1, blockSize * 0.8f, blockSize * 0.8f );
		}
		popMatrix();
	}
	rectMode( CORNER );
...
public void keyReleased()
{
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "StackLogic" };
    if (passedArgs != null) {
